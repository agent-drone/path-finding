...
bool
read_ascii_file (const char *file, OCTREE *tree)
{
...
OCTREE
read_bt_file (const char *file)
{
  return OCTREE (file);
}

Woctomap::Woctomap (float size):
otree (size)
{
}

void
Woctomap::read_from_text (const char *file)
{
  read_ascii_file (file, &otree);
}

void
Woctomap::read_from_bt (const char *file)
{
  otree.readBinary (file);
}

void
Woctomap::add_point (float x, float y, float z, uint8 occupied)
{
